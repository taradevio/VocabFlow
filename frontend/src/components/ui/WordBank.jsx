...
import { useEffect, useState } from "react";
import {
  Badge, Button, TextField, Dialog, RadioCards, Flex, Text, Box
} from "@radix-ui/themes";
...
export const WordBank = () => {
  ...
  const words = Array.isArray(getWords) ? getWords : [];
  const getTotalWords = words.length;

  const deleteDialog = () => setIsDelete(true);

  // ✅ delete by stable key (word), not filtered index
  ...
  const wordsToShow = words
    ...
    .filter((w) => {
      if (getDifficulty === "all") return true;
      return (w.difficulty || "").toLowerCase() === getDifficulty.toLowerCase();
    });
  ...
  return (
    <div className="lg:ps-5 lg:pe-5 my-5">
      <div className="flex items-center justify-between border-1 rounded-lg p-3">
        ...
        <div>
          <span className="block text-end text-4xl font-bold">{getTotalWords}</span>
          <p>Total Words</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center p-3 border-1 rounded-md gap-2">
        ...
        <div className="p-1 flex flex-wrap justify-center gap-2">
          <RadioCards.Root
            defaultValue="all"
            onValueChange={(value) => setGetDifficulty(value)}
          >
            <Flex gap="3">
              <RadioCards.Item value="all"><Box px="2"><Text>All</Text></Box></RadioCards.Item>
              <RadioCards.Item value="beginner"><Box px="5"><Text>Beginner</Text></Box></RadioCards.Item>
              <RadioCards.Item value="intermediate"><Box px="5"><Text>Intermediate</Text></Box></RadioCards.Item>
              <RadioCards.Item value="advanced"><Box px="5"><Text>Advanced</Text></Box></RadioCards.Item>
            </Flex>
          </RadioCards.Root>
        </div>
      </div>
      ...
      {words.length === 0 ? (
        ...
        <div className="rounded-md border-1 w-full text-center p-8 mt-8">
          <h3 className="text-2xl">No words found</h3>
          <p>You may have yet added a word or made a typo in your search</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-8">
          {wordsToShow.map((word, idx) => (
            ...
            >
              ...
              {word.partsOfSpeech && (
                <div><Badge color="bronze">{word.partsOfSpeech}</Badge></div>
              )}
                ...
                <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
                  ...
                  {isDelete && (
                    <Dialog.Content maxWidth="350px">
                      ...
                      <div className="flex justify-between pt-3">
                        <Dialog.Close><Button>No</Button></Dialog.Close>
                        <Button
                          onClick={() => {
                            handleDelete(word.word); // ✅ delete by key
                            setIsOpen(false);
                            setIsDelete(false);
                          }}
                          color="crimson"
